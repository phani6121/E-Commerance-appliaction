...
const VerticalCart = ({ loading, data = [] }) => {
    ...
    return (
        <div className='grid grid-cols-[repeat(auto-fit,minmax(260px,300px))] justify-center md:justify-start md:gap-2 overflow-x-scroll scrollbar-none transition-all ' >


            {
                loading ? (
                    loadingList.map((product, index) => (
                        <div key={product} className='min-w-[280px] md:min-w-[320px] max-w-[320px]  bg-white rounded-sm shadow'>
                    ...
                    data.map((product, index) => (
                        <Link key={index} to={"/product/" + product?._id} className='min-w-[280px] md:min-w-[300px] max-w-[300px]  bg-white rounded-sm shadow' onClick={scrollTop}>
